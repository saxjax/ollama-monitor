...
(() => {
  ...
  const sharedTimeline = () => `${sharedHeader('A', 'Usage timeline', 'One selected month, one stationary detail card, and two graph faces: daily spikes and 30-minute usage.')}${monthRail()}<div class="proto-shared-toolbar"><span class="proto-toolbar-mode">DAILY BARS</span><div class="proto-toolbar-controls"></div></div><div class="proto-a-grid proto-shared-grid">${graphDeck()}${focusDetail()}</div><p class="proto-shared-guide"></p><div class="proto-shared-footer"></div><button class="proto-source" data-variant="${variant === 'A' ? 'B' : 'A'}">${variant === 'A' ? 'OPEN B · INSPECT THIS MONTH’S TIME SLOTS →' : '← RETURN TO A · SPIKE LENS'}</button>${navigator()}${focusLanes()}`;
