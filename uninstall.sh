...
data_dir="$HOME/Library/Application Support/Saxjax Monitor"
...
if [[ "${1:-}" == "--purge" ]]; then
  ...
  printf 'Removed Saxjax Monitor, including locally captured prompt history.\n'
else
  printf 'Removed Saxjax Monitor. Local history remains at %s\n' "$data_dir"
