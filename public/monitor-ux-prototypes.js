...
function calendarDays(month) {
  const safeMonth = /^\d{4}-\d{2}$/.test(month || "")
    ? month
    : localParts(new Date().toISOString()).month;
  const [year, monthNumber] = safeMonth.split("-").map(Number);
  const count = new Date(Date.UTC(year, monthNumber, 0)).getUTCDate();
  return Array.from(
    { length: count },
    (_, index) => `${safeMonth}-${String(index + 1).padStart(2, "0")}`,
  );
}
...
function formatMonth(month, short = false) {
  const safeMonth = /^\d{4}-\d{2}$/.test(month || "")
    ...
    : localParts(new Date().toISOString()).month;
  const [year, monthNumber] = safeMonth.split("-").map(Number);
  return (short ? shortMonthFormatter : monthFormatter).format(
...
function normalizeEvents(snapshot) {
  ...
  return (snapshot?.usageEvents || [])
    .flatMap((event, index) => {
      ...
      const native = Number.isFinite(event.measurements?.nativeUnit?.value)
        ? event.measurements.nativeUnit.value
        : null;
      ...
      return [
        {
          ...
          durationMinutes: Math.max(
            1,
            Math.round((event.timing?.durationMs || 0) / 60000),
          ),
          family: familyOf(event),
          model:
            event.model?.exactLabel || event.model?.family || "Unknown model",
          provider,
          ...
          credits: native,
          tokens: totalTokens,
          inputTokens: Number.isFinite(event.measurements?.inputTokens)
            ? event.measurements.inputTokens
            : null,
          outputTokens: Number.isFinite(event.measurements?.outputTokens)
            ? event.measurements.outputTokens
            : null,
          dollars:
            native == null || !Number.isFinite(providerRule.usdPerNative)
              ? null
              : native * providerRule.usdPerNative,
          prompt:
            event.promptExcerpt ||
            "Prompt excerpt was cleared or not supplied.",
          source: [
            source.client || "Local client",
            source.journalSessionId
              ? `session ${source.journalSessionId}`
              : null,
            source.requestStartedAt || event.timing?.startedAt || instant,
          ]
            .filter(Boolean)
            .join(" · "),
          sessionId:
...
function normalizeLiveRequest(item) {
  ...
  const finishedAt =
    item.finishedAt && Number.isFinite(Date.parse(item.finishedAt))
      ? item.finishedAt
      : null;
  const metrics = item.metrics || {};
  const inputTokens = Number.isFinite(metrics.promptTokens)
    ? metrics.promptTokens
    : null;
  const outputTokens = Number.isFinite(metrics.outputTokens)
  ...
  return {
    ...
    durationMinutes: Math.max(
      1,
      Math.round((Number(metrics.totalDurationNs) || 0) / 60_000_000_000),
    ),
    ...
    prompt:
      ...
      "Live request (prompt excerpt unavailable)",
...
function formatValue(value, unit, compact = false) {
  ...
  if (unit === "dollars")
    return `${compact ? "≈" : "≈ "}$${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  if (unit === "tokens") {
...
function sortSlotsByDateTime(slots, direction) {
  return [...slots].sort((left, right) =>
    globalThis.SaxjaxDateTimeSort.compareValues(
...
function buildView(events, month, unit, anomalyMode = "value", threshold = 0) {
  ...
  const dayMap = new Map(
    days.map((day) => [
      day,
      { day, total: 0, events: [], byOrigin: {}, missing: 0 },
    ]),
  );
  const slots = days.flatMap((day) =>
    Array.from({ length: 48 }, (_, bucket) => ({
      id: `${day}:${bucket}`,
      day,
      bucket,
      total: 0,
  ...
  for (const event of monthEvents) {
    ...
    originTotals.set(
      event.origin,
      (originTotals.get(event.origin) || 0) + value,
    );
    ...
    const startBucket = Math.max(
      0,
      Math.min(47, Math.floor((event.minute || 0) / 30)),
    );
    const endBucket = Math.max(
      ...
      Math.min(
        48,
        Math.ceil(
          ((event.minute || 0) + Math.max(1, event.durationMinutes || 0)) / 30,
        ),
      ),
    );
  ...
  const peakDay = daily.reduce(
    ...
    daily[0] || { total: 0, events: [] },
  );
  const peakSlot = slots.reduce(
    (peak, slot) => (slot.total > peak.total ? slot : peak),
    slots[0] || { total: 0, events: [] },
  );
  const nonZero = slots
    .map((slot) => slot.total)
    .filter(Boolean)
  ...
  const bucketBaselines = Array.from(
    { length: 48 },
    (_, bucket) =>
      median(
        slots
          .filter((slot) => slot.bucket === bucket && slot.total > 0)
          .map((slot) => slot.total),
      ) || occupiedMedian,
  );
  const ranked = slots
    .flatMap((slot, index) => {
      ...
      const jumpPercent =
        ((slot.total - previous) / Math.max(1, previous)) * 100;
      const score =
        anomalyMode === "jump"
          ? jumpPercent
          : anomalyMode === "unusual"
  ...
  const topSlots = ranked
    .slice(0, 12)
    .map((slot, rank) => ({ ...slot, rank: rank + 1 }));
  let running = 0;
  const cumulative = slots.map((slot) => (running += slot.total));
  const originTotalsList = [...originTotals.keys()]
    .map((origin) => ({
      origin,
  ...
  return {
    ...
    topSlots,
    cumulative,
    originTotals: originTotalsList,
    measured,
    missing: monthEvents.length - measured,
    median: occupiedMedian,
    sessions: [...sessions],
  };
}
...
function systemView(state) {
  ...
  const memory = Number.isFinite(system.memoryFree)
    ? 100 - system.memoryFree
    : 0;
  const swap = system.swapTotalMB
    ? (system.swapUsedMB / system.swapTotalMB) * 100
    : 0;
  const loadRatio =
    system.logicalCores && system.load?.[0] != null
      ? system.load[0] / system.logicalCores
      : 0;
  const flags = [cpu >= 85, memory >= 90, loadRatio >= 1, swap >= 70];
  const severity = flags.some(Boolean)
    ? "pressure"
    : cpu >= 65 || memory >= 78 || loadRatio >= 0.75
      ? "busy"
      : "clear";
...
function commonHeader(model, concept, subtitle) {
  ...
  return `<header class="mux-head">
    ...
    <div class="mux-period">
      ...
      <div class="mux-unit" aria-label="Usage unit">${[
        ["tokens", "Tokens"],
        ["credits", "Provider units"],
        ["dollars", "Money"],
      ]
        .map(
          ([key, label]) =>
            `<button data-action="unit" data-unit="${key}" class="${unit === key ? "is-active" : ""}" ${model.capabilities[key].available ? "" : `disabled title="${escapeHtml(model.capabilities[key].note)}"`}>${label}</button>`,
        )
        .join("")}</div>
    </div>
...
function controlDeck(model) {
  const activeFilters =
    [model.filters.provider, model.filters.origin, model.filters.model].filter(
      (value) => value !== "all",
    ).length + (model.filters.search ? 1 : 0);
  const planningVariant = ["F", "H", "I"].includes(model.variant);
  const referenceControl = planningVariant
    ? `<label class="mux-reference-select">Reference month<select data-action="reference-select"><option value="">No comparison</option>${model.months
        .filter((item) => item !== model.month)
        .map(
          (item) =>
            `<option value="${item}" ${model.referenceMonth === item ? "selected" : ""}>${formatMonth(item)}</option>`,
        )
        .join("")}</select></label>`
    : `<div><span>Reference month</span>${model.months
        .filter((item) => item !== model.month)
        .map(
          (item) =>
            `<button data-action="reference" data-month="${item}" class="${model.referenceMonth === item ? "is-active" : ""}">${formatMonth(item, true)}</button>`,
        )
        .join("")}</div>`;
  const compactSummary = [
    ...
    model.referenceMonth
      ? `vs ${formatMonth(model.referenceMonth, true)}`
      : `${model.zoom} view`,
  ].join(" · ");
  return `<section class="mux-controls ${model.controlsOpen ? "is-open" : ""}" aria-label="Timeline controls">
    ...
    <div class="mux-control-row">
      ${referenceControl}
      <div><span>Rank spikes by</span>${[
        ["value", "Total"],
        ["jump", "% jump"],
        ["unusual", "Unusual for time"],
      ]
        .map(
          ([key, label]) =>
            `<button data-action="anomaly" data-mode="${key}" class="${model.anomalyMode === key ? "is-active" : ""}">${label}</button>`,
        )
        .join(
          "",
        )}<label class="mux-threshold">Minimum <input data-action="threshold" type="number" min="0" step="1" value="${model.threshold || ""}" placeholder="0" /></label></div>
      <div><span>Inspection scale</span>${[
        ["month", "Month"],
        ["day", "Day"],
        ["slot", "30 min"],
        ["session", "Session"],
        ...
        .map(
          ([key, label]) =>
            `<button data-action="zoom" data-zoom="${key}" class="${model.zoom === key ? "is-active" : ""}" ${key === "session" && !model.selectedSessionId ? "disabled" : ""}>${label}</button>`,
        )
        .join("")}</div>
      <div><span>Table date/time</span>${[
        ["desc", "Newest first ↓"],
        ["asc", "Oldest first ↑"],
      ]
        .map(
          ([key, label]) =>
            `<button data-action="sort" data-sort="${key}" class="${model.sortDirection === key ? "is-active" : ""}" aria-pressed="${model.sortDirection === key}">${label}</button>`,
        )
        .join("")}</div>
    </div>
...
function systemStrip(model, compact = false) {
  ...
  const metric = (label, value, percent, live) =>
    `<div class="mux-vital"><span>${label}</span><b data-live="${live}">${value}</b><i><em data-live-bar="${live}" style="--fill:${Math.max(0, Math.min(100, percent))}%"></em></i></div>`;
...
function copilotNumber(value, suffix = "") {
  return Number.isFinite(value)
    ? `${value.toLocaleString(undefined, { maximumFractionDigits: suffix ? 1 : 0 })}${suffix}`
    : "—";
}
...
function burnDecisionPanel(model, treatment) {
  ...
  const pacePercent = hasFactor
    ? Math.min(180, (1 / Math.max(0.01, factor)) * 100)
    : 0;
  const targetPercent = hasFactor ? 100 : 0;
  const projection = Number.isFinite(forecast.projectedMonthCredits)
    ? forecast.projectedMonthCredits
    : null;
  const price = Number.isFinite(usage.tokenPriceUsdPerMillion)
    ? `$${usage.tokenPriceUsdPerMillion.toLocaleString()}/1M`
    : "Per-model rates";
  const action =
    state === "over"
...
function copilotAccountPanel(model, treatment = "ledger") {
  ...
  const estimated = Boolean(
    usage.usageEstimate || usage.status === "estimated",
  );
  const source =
    usage.status === "ready"
  ...
  const updated = usage.updatedAt
    ...
    : "—";
  const budget = Number.isFinite(usage.monthlyBudgetUsd)
    ? `$${usage.monthlyBudgetUsd.toFixed(2)}`
    : "Not set";
  const budgetValue = Number.isFinite(usage.estimatedCost)
    ? `$${usage.estimatedCost.toFixed(2)}`
    : "—";
  const costLabel = estimated ? "estimated" : "billed";
  const creditValue = Number.isFinite(usage.usedCredits)
    ? `${estimated ? "≈" : ""}${copilotNumber(usage.usedCredits)}`
    : "—";
  const form = (label, placeholder, note) =>
    `<div class="mux-copilot-form"><label>${label}<div><span>$</span><input placeholder="${placeholder}" aria-label="${label}" /><button type="button">Save</button></div><small>${note}</small></label></div>`;
...
function throughputPanel(
  model,
  compact = false,
  sourceVariant = model.variant,
) {
  const system = model.system;
  const requestTotal = Math.max(0, system.total);
  const activeShare = requestTotal
    ? Math.min(100, (system.active / requestTotal) * 100)
    : 0;
  const errorShare = requestTotal
    ? Math.min(100, (system.errors / requestTotal) * 100)
    : 0;
...
function truthPanel(model) {
  const capabilities = ["tokens", "credits", "dollars"]
    .map((unit) => [unit, model.capabilities[unit]])
    .map(
      ([unit, item]) =>
        `<div><span>${unit === "credits" ? "Provider units" : unit}</span><b>${item.available ? item.confidence : "Unavailable"}</b><small>${escapeHtml(item.note)}</small></div>`,
    )
    .join("");
...
function dailyChart(model, height = 240) {
  ...
  const peak = Math.max(
    ...
    ...referenceDaily.map((day) => day.total),
  );
    ...
    ${daily
      .map((day, index) => {
        ...
        const referenceHeight =
          ((referenceDaily[index]?.total || 0) / peak) * (bottom - 12);
        const shadow = referenceHeight
          ? `<rect class="mux-reference-mark" x="${x - 1}" y="${bottom - referenceHeight}" width="${Math.max(3, column - 1)}" height="${referenceHeight}"/>`
          : "";
        const pieces = model.view.originTotals
          .map(({ origin }) => {
            const value = day.byOrigin[origin] || 0;
            const piece = (value / peak) * (bottom - 12);
            used += piece;
            return piece
              ? `<rect x="${x}" y="${bottom - used}" width="${Math.max(2, column - 3)}" height="${piece}" fill="${ORIGIN_COLORS[origin] || ORIGIN_COLORS.other}"/>`
              : "";
          })
          .join("");
        const missing = day.missing
          ? `<line class="mux-missing-mark" x1="${x}" x2="${x + Math.max(2, column - 3)}" y1="${bottom - used - 4}" y2="${bottom - used - 4}"/>`
          : "";
...
function slotChart(
  ...
  const source = monthly
    ? model.view.slots
    : model.view.slots.filter((slot) => slot.day === day);
  const referenceSource = model.referenceView
    ? monthly
      ? model.referenceView.slots
      : model.referenceView.slots.filter(
          (slot) => slot.day.slice(-2) === day.slice(-2),
        )
    : [];
  const width = 1120;
  const left = 48;
  ...
  const peak = Math.max(
    1,
    ...source.map((slot) => slot.total),
    ...referenceSource.map((slot) => slot.total),
  );
  return `<svg class="mux-chart mux-slot-chart" tabindex="0" data-timeline-keyboard="true" viewBox="0 0 ${width} ${height}" role="img" aria-label="${monthly ? "Month" : "Selected day"} usage in 30-minute slots. Use arrow keys to move the selection.">
    ...
    ${source
      .map((slot, index) => {
        ...
        let used = 0;
        const referenceHeight =
          ((referenceSource[index]?.total || 0) / peak) * (bottom - 12);
        const shadow = referenceHeight
          ? `<rect class="mux-reference-mark" x="${x}" y="${bottom - referenceHeight}" width="${Math.max(0.8, column)}" height="${referenceHeight}"/>`
          : "";
        const pieces = model.view.originTotals
          .map(({ origin }) => {
            ...
            used += piece;
            return piece
              ? `<rect x="${x}" y="${bottom - used}" width="${Math.max(0.7, column - (monthly ? 0.15 : 2))}" height="${piece}" fill="${ORIGIN_COLORS[origin] || ORIGIN_COLORS.other}"/>`
              : "";
          })
          .join("");
        const missing = slot.missing
          ? `<line class="mux-missing-mark" x1="${x}" x2="${x + Math.max(0.7, column)}" y1="${bottom - used - 3}" y2="${bottom - used - 3}"/>`
          : "";
        return `${shadow}${pieces}${missing}<rect class="mux-hit ${slot.id === model.selectedSlotId ? "is-selected" : ""}" data-action="slot" data-slot="${slot.id}" x="${x}" y="10" width="${Math.max(0.7, column)}" height="${bottom - 8}"/>`;
      })
      .join("")}
...
function dominantOrigin(item) {
  return (
    Object.entries(item.byOrigin || {}).sort(
...
function flightPathChart(model, height = 280, monthly = true) {
  const source = monthly
    ? model.view.daily
    : model.view.slots.filter((slot) => slot.day === model.selectedDay);
  ...
  const peak = Math.max(
    1,
    ...source.map((item) => item.total),
    ...referenceSource.map((item) => item.total),
  );
  ...
  const area = points.length
    ? `${path} L${points.at(-1).x.toFixed(2)} ${bottom} L${points[0].x.toFixed(2)} ${bottom} Z`
    : "";
  const referencePath = referenceSource
    .map(point)
    .map(
      ({ x, y }, index) =>
        `${index ? "L" : "M"}${x.toFixed(2)} ${y.toFixed(2)}`,
    ...
    ? source
        .map((item, index) =>
          index % 3 === 0 || index === source.length - 1
            ? `<text x="${left + index * step - 4}" y="${height - 8}">${item.day.slice(-2)}</text>`
            : "",
        )
        .join("")
...
function seismographChart(model, height = 330) {
  ...
  const trace = source
    .map((slot, index) => {
      ...
      const amplitude = (slot.total / peak) * (height * 0.36);
  ...
  return `<svg class="mux-chart mux-seismograph-chart" tabindex="0" data-timeline-keyboard="true" viewBox="0 0 ${width} ${height}" role="img" aria-label="Seismograph trace. Vertical springs are exact interval contributions.">
    <g class="mux-seismic-grid"><line x1="${left}" y1="${center}" x2="${width - right}" y2="${center}"/><line x1="${left}" y1="${height * 0.14}" x2="${width - right}" y2="${height * 0.14}"/><line x1="${left}" y1="${height * 0.86}" x2="${width - right}" y2="${height * 0.86}"/></g>
    <path class="mux-seismic-trace" d="${trace}"/>${springs}
    <text x="2" y="${height * 0.14 + 4}">MAX SHOCK · ${escapeHtml(formatLabeledValue(peak, model.unit, true))}</text><text x="2" y="${center + 4}">REST · 0 ${escapeHtml(unitLabel(model.unit))}</text>
  </svg>`;
}
...
function orbitChart(model, height = 440) {
  ...
  const cx = 540;
  const cy = height / 2;
  const orbit = Math.min(168, height * 0.37);
  ...
  const nodes = source
    .map((item, index) => {
      ...
      const radius = orbit + (activity - 0.35) * 46;
  ...
  return `<svg class="mux-chart mux-orbit-chart" tabindex="0" data-timeline-keyboard="true" viewBox="0 0 ${width} ${height}" role="img" aria-label="Orbital usage network. Distance and node size encode usage.">
    ...
    <text x="${cx}" y="20" text-anchor="middle">${monthly ? "01" : "00:00"}</text><text x="${width - 330}" y="${cy + 4}">${monthly ? String(Math.ceil(source.length * 0.25)).padStart(2, "0") : "06:00"}</text><text x="${cx}" y="${height - 8}" text-anchor="middle">${monthly ? String(Math.ceil(source.length * 0.5)).padStart(2, "0") : "12:00"}</text><text x="290" y="${cy + 4}" text-anchor="end">${monthly ? String(Math.ceil(source.length * 0.75)).padStart(2, "0") : "18:00"}</text>
  </svg>`;
}
...
function bubbleTimeline(model, day = model.selectedDay, height = 190) {
  ...
  return `<svg class="mux-chart mux-bubble-chart" tabindex="0" data-timeline-keyboard="true" viewBox="0 0 ${width} ${height}" role="img" aria-label="Bubble constellation by source and time.">
    ${origins.map((origin, index) => `<line x1="${left}" y1="${18 + row * (index + 0.5)}" x2="${width - right}" y2="${18 + row * (index + 0.5)}"/><text x="2" y="${22 + row * (index + 0.5)}">${escapeHtml((ORIGIN_LABELS[origin] || origin).slice(0, 7).toUpperCase())}</text>`).join("")}${circles}
...
function usageFlowChart(model, height = 320) {
  ...
  const slots = [...candidates]
    ...
    .sort(
      (left, right) =>
        left.day.localeCompare(right.day) || left.bucket - right.bucket,
    );
  const origins = [
    ...new Set(slots.flatMap((slot) => Object.keys(slot.byOrigin))),
  ];
  const width = 1120;
  const leftX = 120;
  const middleX = 600;
  ...
  if (!slots.length)
    return `<div class="mux-flow-empty">No measured flow in this selection.</div>`;
  const originY = new Map(
    origins.map((origin, index) => [
      origin,
      top + usable * ((index + 0.5) / origins.length),
    ]),
  );
  const slotY = new Map(
    slots.map((slot, index) => [
      slot.id,
      top + usable * ((index + 0.5) / slots.length),
    ]),
  );
  const links = slots
    .flatMap((slot) =>
...
function cumulativeChart(model, height = 240) {
  ...
  const referenceByAlignedSlot = new Map(
    (model.referenceView?.slots || []).map((slot) => [
      `${slot.day.slice(-2)}:${slot.bucket}`,
      slot.total,
    ]),
  );
  const referenceValues = model.referenceView
    ? model.view.slots.map(
        (slot) =>
          (referenceRunning +=
            referenceByAlignedSlot.get(
              `${slot.day.slice(-2)}:${slot.bucket}`,
            ) || 0),
      )
    : [];
  ...
  const path = values
    .map(
      (value, index) =>
        `${index ? "L" : "M"} ${left + index * step} ${bottom - (value / peak) * (bottom - 14)}`,
    )
    .join(" ");
  ...
  const referencePath = referenceValues
    .map(
      (value, index) =>
        `${index ? "L" : "M"} ${left + index * referenceStep} ${bottom - (value / peak) * (bottom - 14)}`,
    )
    .join(" ");
  const contributions = model.view.slots
    .map((slot, index) =>
      slot.total
        ? `<line data-action="slot" data-slot="${slot.id}" x1="${left + index * step}" y1="${bottom}" x2="${left + index * step}" y2="${bottom - (slot.total / model.view.peakSlot.total) * 42}"/>`
        : "",
    )
    .join("");
...
function incidentList(model, limit = 7) {
  const score = (slot) =>
    model.anomalyMode === "jump"
      ? `${Math.round(slot.jumpPercent).toLocaleString()}% vs previous slot`
      : model.anomalyMode === "unusual"
        ? `${slot.multiple.toFixed(1)}× normal for this time`
        : `${slot.multiple.toFixed(1)}× occupied-slot median`;
  const slots = sortSlotsByDateTime(
    model.view.topSlots.slice(0, limit),
    model.sortDirection,
  );
  return `<ol class="mux-incidents" aria-label="Top usage intervals, ${sortLabel(model.sortDirection).toLowerCase()}">${slots
    .map(
      (
        slot,
      ) => `<li class="${slot.id === model.selectedSlotId ? "is-selected" : ""}">
...
function evidence(model, limit = 24) {
  const selected =
    model.view.slots.find((slot) => slot.id === model.selectedSlotId) ||
    model.view.peakSlot;
  const session = model.selectedSessionId
    ? model.view.monthEvents.filter(
        (event) => event.sessionId === model.selectedSessionId,
      )
    : null;
  const events = [...(session?.length ? session : selected?.events || [])].sort(
    (left, right) =>
      globalThis.SaxjaxDateTimeSort.compareDateTimes(
        left,
        right,
  ...
  const value = events
    .filter((event) => Number.isFinite(event[model.unit]))
    .reduce((sum, event) => sum + event[model.unit], 0);
  const missing = events.filter(
    (event) => !Number.isFinite(event[model.unit]),
  ).length;
  return `<section class="mux-evidence" id="mux-forensics">
    ...
    <div class="mux-evidence-list">${
      events
        .map((event, index) => {
          const exchange = capturedExchange(model, event);
          const incoming = exchange
            ? readableCapture(
                exchange.inputContext,
                readableCapture(
                  exchange.transformedPrompt,
                  readableCapture(exchange.prompt, event.prompt),
                ),
              )
            : "";
          const outgoing = exchange
            ? readableCapture(exchange.response, "")
            : "";
          const reasoning = exchange
            ? readableCapture(exchange.thinking, "")
            : "";
          const canLoadJournal = Boolean(
            event.captureClient && event.journalSessionId && event.requestId,
          );
        ...
        .join("") ||
      `<p class="mux-empty">No measured request started in this interval.</p>`
    }</div>
  </section>`;
}
...
function sessionLanes(model) {
  const dayEvents = model.view.monthEvents.filter(
    (event) => event.day === model.selectedDay,
  );
  ...
  const lanes = [...groups.values()].sort(
    (left, right) =>
      globalThis.SaxjaxDateTimeSort.compareValues(
        left.start,
        right.start,
...
function resourceCorrelation(model) {
  const daySlots = model.view.slots.filter(
    (slot) => slot.day === model.selectedDay,
  );
  const samples = model.liveSamples.filter(
    (sample) => localParts(sample.at).day === model.selectedDay,
  );
  const sampleBins = (read) =>
    Array.from({ length: 48 }, (_, bucket) => {
      const matches = samples
        .filter(
          (sample) => Math.floor(localParts(sample.at).minute / 30) === bucket,
        )
        .map(read)
        .filter(Number.isFinite);
  ...
  const localUsage = daySlots.map((slot) =>
    slot.events
      .filter(
        (event) => event.provider === "ollama" || event.origin === "ollama",
      )
      .reduce((sum, event) => sum + unitValue(event, model.unit), 0),
  );
  const throughput = daySlots.map((slot) =>
    slot.events.reduce(
      (sum, event) =>
        sum +
        (Number.isFinite(event.outputTokens) && event.durationMinutes
          ? event.outputTokens / (event.durationMinutes * 60)
          : 0),
      0,
    ),
  );
...
function VariantC(model) {
  return `<div class="mux mux-c">
    <header class="mux-c-head"><div><span>LOCAL INFERENCE / CONTROL ROOM</span><h1>Machine room</h1></div><div class="mux-c-clock">${formatMonth(model.month)}<b>${formatValue(model.view.total, model.unit)}</b><small>${unitLabel(model.unit)}</small></div><div class="mux-c-controls">${
      commonHeader(model, "", "")
        .match(
          /<div class="mux-period">[\s\S]*?<\/div>\s*<div class="mux-total">/,
        )?.[0]
        .replace('<div class="mux-total">', "") || ""
    }</div></header>
    ${controlDeck(model)}
...
function dayFilm(day, model) {
  ...
  return `<button class="mux-film-day ${day.day === model.selectedDay ? "is-selected" : ""}" data-action="day" data-day="${day.day}">
    ...
    <svg class="mux-film-wave" viewBox="0 0 240 54" aria-hidden="true">${slots
      .map((slot, index) => {
        const radius = slot.total
          ? 1.7 + Math.sqrt(slot.total / peak) * 6.5
          : 0.75;
        const y = 27 + Math.sin(index * 0.92) * 11;
...
function VariantE(model) {
  ...
  return `<div class="mux mux-e">
    ...
    <div class="mux-e-toolbar">${
      commonHeader(model, "", "")
        .match(
          /<div class="mux-period">[\s\S]*?<\/div>\s*<div class="mux-total">/,
        )?.[0]
        .replace('<div class="mux-total">', "") || ""
    }<span>${model.view.sessions.length.toLocaleString()} sessions examined</span><strong>${model.capabilities[model.unit].available ? formatValue(model.view.total, model.unit) : "Unavailable"} ${unitLabel(model.unit)}</strong></div>
    ${controlDeck(model)}
    ${throughputPanel(model, true)}
...
function monthPlanning(model) {
  ...
  const readings = model.manualReadings
    .filter(
      (reading) =>
        reading.date.startsWith(model.month) && reading.unit === model.unit,
    )
  ...
  const localValue =
    model.unit === "credits"
      ? copilot.usedCredits == null
        ? Number.NaN
        : Number(copilot.usedCredits)
      : model.unit === "dollars"
        ? copilot.estimatedCost == null
          ? Number.NaN
          : Number(copilot.estimatedCost)
        : Number(copilot.observedInputTokens || 0) +
          Number(copilot.observedOutputTokens || 0);
  const timelineValue = model.capabilities[model.unit].available
    ? model.view.total
    : Number.NaN;
  const fallbackValue =
    model.month === currentMonth && Number.isFinite(localValue)
      ? localValue
      : timelineValue;
  const used = latest
    ? latest.value
  ...
  const capturedVelocity =
    model.unit === "credits"
      ? Number(forecast.creditsPerDay)
      : model.unit === "dollars"
        ? Number(forecast.creditsPerDay) / 100
        : (Number(forecast.inputTokens || 0) +
            Number(forecast.outputTokens || 0)) /
          Math.max(1, Number(forecast.observationHours || 0) / 24);
  const completedDays = model.view.daily.slice(0, elapsed);
  const timelineObserved = completedDays.reduce(
    (sum, day) => sum + day.total,
    0,
  );
  const recentDays = completedDays.slice(-Math.min(7, completedDays.length));
  const recentWeight = recentDays.reduce(
    (sum, _day, index) => sum + index + 1,
    0,
  );
  const weightedTimelineVelocity = recentWeight
    ? recentDays.reduce((sum, day, index) => sum + day.total * (index + 1), 0) /
      recentWeight
    : 0;
  const timelineScale = timelineObserved > 0 ? used / timelineObserved : 1;
  const observedVelocity =
    weightedTimelineVelocity > 0
      ? weightedTimelineVelocity * timelineScale
      : used / elapsed;
  let velocity =
    Number.isFinite(capturedVelocity) && capturedVelocity > 0
      ? capturedVelocity
      : observedVelocity;
  let velocitySource =
    Number.isFinite(capturedVelocity) && capturedVelocity > 0
      ? "captured request velocity"
      : "weighted recent daily usage";
  if (latest && previous) {
    const span = Math.max(
      1,
      (Date.parse(`${latest.date}T12:00:00`) -
        Date.parse(`${previous.date}T12:00:00`)) /
        86_400_000,
    );
    ...
    velocitySource = latest
      ? "provider reading ÷ elapsed days"
      : "month average";
  }
  const additions = model.budgetAdditions
    .filter(
      (addition) =>
        addition.month === model.month && addition.unit === model.unit,
    )
    .sort((left, right) => left.date.localeCompare(right.date));
  const baseBudget = Math.max(
    model.unit === "dollars" ? 0.01 : 1,
    model.baseBudgets[model.unit],
  );
  const addedBudget = additions.reduce(
    (sum, addition) => sum + addition.value,
    0,
  );
  const target = baseBudget + addedBudget;
  const sustainable = remainingDays
    ? Math.max(0, target - used) / remainingDays
    : 0;
  ...
  const ratio =
    sustainable > 0 ? velocity / sustainable : velocity > 0 ? Infinity : 0;
  const state =
    finishValue > target * 1.05
      ? "over"
      : finishValue < target * 0.78
        ? "under"
        : "aligned";
  const delta =
    latest && Number.isFinite(localValue) ? latest.value - localValue : null;
  ...
  const boosterPenalty = usedBooster
    ? Math.min(
        ...
        4 +
          additions.length * 3 +
          Math.round((addedBudget / Math.max(1, baseBudget)) * 8),
      )
    : 0;
  ...
  const overrunPenalty =
    unapprovedOverrun > 0
      ? Math.min(
          50,
          10 + Math.round((unapprovedOverrun / Math.max(1, target)) * 40),
        )
      : 0;
  const ratingScore = Math.max(
    0,
    accuracyScore - boosterPenalty - overrunPenalty,
  );
  const ratingGrade =
    ratingScore >= 95
      ? "S"
    ...
    unapprovedOverrun > 0
      ? unapprovedOverrunRatio >= 3
        ...
        : unapprovedOverrunRatio >= 0.5
          ...
          : unapprovedOverrunRatio >= 0.1
            ? "⚠️ OVER BUDGET · RECOVERABLE"
            : "🟠 SLIGHT OVERRUN · NEAR MISS"
      : finishValue < baseBudget * 0.8
        ? isFinal
          ? "🥶 FINAL · BUDGET LEFT BEHIND"
          : "🥶 PACE FORECAST · RUNNING TOO COLD"
        : usedBooster
  ...
  const dramaQuip =
    unapprovedOverrun > 0
      ? unapprovedOverrunRatio >= 3
        ? isFinal
          ? `You blasted ${formatLabeledValue(unapprovedOverrun, model.unit)} past every approved limit. Your budget would like a restraining order.`
          : `Current velocity is blasting you ${formatLabeledValue(unapprovedOverrun, model.unit)} past every approved limit. Your budget is filing paperwork.`
        : unapprovedOverrunRatio >= 1
          ? isFinal
            ? `You finished ${formatLabeledValue(unapprovedOverrun, model.unit)} over the approved line—more than an entire extra budget. Please put the flamethrower down.`
            : `Current velocity carries you ${formatLabeledValue(unapprovedOverrun, model.unit)} over the approved line—more than an entire extra budget. Please lower the flamethrower.`
          : unapprovedOverrunRatio >= 0.5
            ? isFinal
              ? `You ran ${formatLabeledValue(unapprovedOverrun, model.unit)} over the approved line. This was a real miss, but at least the building is still standing.`
              : `Current velocity runs ${formatLabeledValue(unapprovedOverrun, model.unit)} over the approved line. This is a real miss, but the brakes still work.`
            : unapprovedOverrunRatio >= 0.1
              ? isFinal
                ? `You overshot by ${formatLabeledValue(unapprovedOverrun, model.unit)}. Not madness—just enough enthusiasm to earn an awkward meeting.`
                : `Current velocity overshoots by ${formatLabeledValue(unapprovedOverrun, model.unit)}. Not madness—just enough enthusiasm to schedule an awkward meeting.`
              : isFinal
                ? `You slipped ${formatLabeledValue(unapprovedOverrun, model.unit)} over the line. Annoying, fixable, and definitely not a catastrophe.`
                : `Current velocity slips ${formatLabeledValue(unapprovedOverrun, model.unit)} over the line. Annoying, fixable, and not yet a catastrophe.`
      : finishValue < baseBudget * 0.8
        ...
        : usedBooster
          ? isFinal
            ? `${additions.length} approved booster${additions.length === 1 ? "" : "s"} kept the finish legal. The scoreboard still charged ${boosterPenalty} points for the rescue.`
            : `${additions.length} approved booster${additions.length === 1 ? " is" : "s are"} keeping the forecast legal. The scoreboard currently charges ${boosterPenalty} points for the rescue.`
          : distanceRatio <= 0.05
            ? isFinal
              ? `You finished within 5% of the original budget: no booster, no excuses. Annoyingly excellent.`
              : `Current velocity lands within 5% of the original budget: no booster, no excuses. Annoyingly excellent.`
            : finishValue < baseBudget
              ? isFinal
                ? `You finished ${formatLabeledValue(baseBudget - finishValue, model.unit)} short of the original budget. Caution was not the same as control.`
                : `Current velocity lands ${formatLabeledValue(baseBudget - projection, model.unit)} short of the original budget. Caution is not the same as control.`
              : isFinal
                ? `You finished ${formatLabeledValue(finishValue - baseBudget, model.unit)} over the original budget. Close enough to survive, far enough to get side-eye.`
                : `Current velocity lands ${formatLabeledValue(projection - baseBudget, model.unit)} over the original budget. Close enough to recover, far enough to get side-eye.`;
  const averageVelocity = used / elapsed;
  const velocityTrendRatio =
    averageVelocity > 0 ? scenarioVelocity / averageVelocity : 1;
  const velocityTrend =
    velocityTrendRatio <= 0.65
      ? "cooling-fast"
      : velocityTrendRatio <= 0.85
        ? "cooling"
        : velocityTrendRatio >= 1.5
          ? "accelerating-hard"
          : velocityTrendRatio >= 1.15
            ? "accelerating"
            : "steady";
  const trendLabel =
    velocityTrend === "cooling-fast"
      ? "COOLING FAST"
      : velocityTrend === "cooling"
        ? "COOLING"
        : velocityTrend === "accelerating-hard"
          ? "ACCELERATING HARD"
  ...
  const paceStory = isFinal
    ...
    : finishValue > target && velocityTrendRatio < 0.85
      ? `Still projected over budget, but recent velocity is cooling. Hold the lower pace.`
      : finishValue < baseBudget * 0.8 && velocityTrendRatio > 1.15
  ...
  const efficiencyRecent = efficiencyWindow
    ? completedDays.slice(-efficiencyWindow)
    : [];
  const efficiencyPrevious = efficiencyWindow
    ? completedDays.slice(-efficiencyWindow * 2, -efficiencyWindow)
    : [];
  const windowStats = (days) => ({
    value: days.reduce((sum, day) => sum + day.total, 0),
    requests: days.reduce((sum, day) => sum + day.events.length, 0),
  });
  const recentStats = windowStats(efficiencyRecent);
  const previousStats = windowStats(efficiencyPrevious);
  const recentPerRequest = recentStats.requests
    ? recentStats.value / recentStats.requests
    : 0;
  const previousPerRequest = previousStats.requests
    ? previousStats.value / previousStats.requests
    : 0;
  const efficiencyRatio =
    previousPerRequest > 0 ? recentPerRequest / previousPerRequest : 1;
  const efficiencySignal =
    efficiencyWindow >= 2 &&
    recentStats.requests >= 5 &&
    previousStats.requests >= 5 &&
    efficiencyRatio <= 0.78
      ? {
          percent: Math.round((1 - efficiencyRatio) * 100),
          recentPerRequest,
          previousPerRequest,
        }
      : null;
  const ratingBasis = isFinal
    ? "FINAL ACTUAL COST"
    : "CURRENT VELOCITY FORECAST";
  const ratingBasisDetail = isFinal
    ? "Month closed: score uses actual final cost."
    : `Month open: score uses projected ${formatLabeledValue(projection, model.unit)} month end.`;
  return {
    readings,
    latest,
    previous,
...
const PROMPT_RULES = {
  broad: {
    label: "Unbounded ask",
    advice:
      "Name the exact outcome, files, and stopping condition before the agent explores.",
  },
  repeated: {
    label: "Repeated context",
    advice:
      "Remove duplicate lines and point at one canonical source instead of pasting it again.",
  },
  oversized: {
    label: "Context avalanche",
    advice:
      "Split discovery from implementation and carry forward a short verified brief.",
  },
  contract: {
    label: "No output contract",
    advice:
      "Specify the expected artifact, verification, and what a finished answer must contain.",
  },
  ratio: {
    label: "Input-heavy turn",
    advice:
      "Start a clean task or summarize accumulated context when input dwarfs useful output.",
  },
};

function promptHygiene(model) {
  const incidents = [];
  for (const event of model.view.monthEvents.filter(
    (item) =>
      item.provider === "github-copilot" ||
      item.origin.includes("vscode") ||
      item.origin === "copilot-cli",
  )) {
    ...
    const rules = [];
    if (
      /\b(make it better|do everything|fix everything|help me|improve this|whatever is needed)\b/i.test(
        prompt,
      )
    )
      rules.push("broad");
    if (duplicateLines > 0) rules.push("repeated");
    if (prompt.length > 1800 || Number(event.inputTokens) > 12_000)
      rules.push("oversized");
    if (
      prompt.length > 120 &&
      !/\b(return|output|produce|create|implement|write|explain|list|verify|test|format|file|component|function)\b/i.test(
        prompt,
      )
    )
      rules.push("contract");
    if (
      Number(event.inputTokens) > 4_000 &&
      Number(event.inputTokens) >
        Math.max(1, Number(event.outputTokens) || 0) * 18
    )
      rules.push("ratio");
    ...
    incidents.push({
      ...
      score:
        rules.length * 2 +
        Math.log10(
          Math.max(10, Number(event.inputTokens) || prompt.length / 3.4),
        ),
    });
  }
  incidents.sort((left, right) => right.score - left.score);
  const counts = Object.keys(PROMPT_RULES).map((key) => ({
    key,
    count: incidents.filter((incident) => incident.rules.includes(key)).length,
  }));
  const flaggedUsage = incidents.reduce(
    (sum, incident) => sum + unitValue(incident.event, model.unit),
    0,
  );
...
function capturedExchange(model, event) {
  ...
  const exact =
    traffic.find((item) => item.id === event.id) ||
    traffic.find(
      (item) =>
        item.session?.id === event.sessionId &&
        item.startedAt === event.startedAt,
    ) ||
    traffic.find((item) => item.startedAt === event.startedAt) ||
    null;
  if (exact) return exact;
  const sessionMatches = traffic.filter(
    (item) =>
      item.session?.id === event.sessionId ||
  ...
  const measurementLabel =
    model.unit === "dollars"
      ...
      : model.unit === "tokens"
        ...
        : "Credits used";
  const step =
    model.unit === "dollars" ? ".01" : model.unit === "tokens" ? "1" : ".1";
  const rows = plan.readings
    .map(
      (reading) =>
        `<li><time>${reading.date.slice(-2)}</time><b>${formatValue(reading.value, model.unit)}</b><span>${unitLabel(model.unit)}</span><button data-action="remove-reading" data-reading-id="${reading.id}" aria-label="Remove ${reading.date} reading">×</button></li>`,
    )
    .join("");
...
function courseControls(model, treatment) {
  ...
  const suggestedAdditionDate = model.budgetAdditionDate.startsWith(model.month)
    ...
    : `${model.month}-${String(Math.min(plan.daysInMonth, new Date().getDate())).padStart(2, "0")}`;
  const additionRows = plan.additions
    .map(
      (addition, index) =>
        `<li><time>${addition.date.slice(-2)}</time><span>Booster ${index + 1} used</span><b>+${formatValue(addition.value, model.unit, true)}</b><button data-action="remove-budget-addition" data-budget-addition-id="${addition.id}" aria-label="Remove booster added ${addition.date}">×</button></li>`,
    )
    .join("");
  const selectedPace =
    model.paceScenario === 100
      ? "current pace"
      : `${model.paceScenario}% of current pace`;
    ...
    <ol>${
      hygiene.incidents
        ...
        .map((incident) => {
          ...
          const incoming = readableCapture(
            exchange?.inputContext,
            readableCapture(
              exchange?.transformedPrompt,
              readableCapture(exchange?.prompt, incident.event.prompt),
            ),
          );
          const outgoing = readableCapture(
            exchange?.response,
            "The complete output is not retained in the normalized historic timeline for this request.",
          );
        ...
        .join("") ||
      '<li class="is-empty">The heuristic only sees locally captured excerpts and request proportions. It will stay quiet rather than invent a problem.</li>'
    }</ol>
  </section>`;
}

function planningNavigator(model) {
  return `<div class="mux-plan-nav"><div class="mux-view-month"><span>Viewed month</span><div>${model.months.map((item) => `<button data-action="view-month" data-month="${item}" class="${item === model.month ? "is-active" : ""}" aria-pressed="${item === model.month}">${formatMonth(item, true)}</button>`).join("")}</div></div><div class="mux-plan-units" role="group" aria-label="Usage unit">${[
    ["credits", "Credits"],
    ["tokens", "Tokens"],
    ["dollars", "Money"],
  ]
    .map(
      ([key, label]) =>
        `<button data-action="unit" data-unit="${key}" class="${model.unit === key ? "is-active" : ""}" ${model.capabilities[key].available ? "" : `disabled title="${escapeHtml(model.capabilities[key].note)}"`}>${label}</button>`,
    )
    .join("")}</div></div>`;
}
...
function gardenChart(model, height = 520) {
  ...
  const peak = Math.max(
    1,
    ...source.map((day) => day.total),
    plan.scenarioVelocity,
    plan.sustainable,
  );
  const step = plotWidth / Math.max(1, source.length);
  const latestDay = plan.latest
    ? Number(plan.latest.date.slice(-2))
    : plan.elapsed;
  const weedsByDay = Object.fromEntries(
    hygiene.incidents.map((incident) => [
      incident.event.day,
      hygiene.incidents.filter((item) => item.event.day === incident.event.day)
        .length,
    ]),
  );
  const plants = source
    .map((day, index) => {
      ...
      return `<g class="mux-garden-plant ${future ? "is-future" : ""} ${selected ? "is-selected" : ""}" data-action="day" data-day="${day.day}"><path class="mux-garden-stem" d="M${x} ${ground} C${x - 5} ${ground - h * 0.45},${x + 7} ${ground - h * 0.72},${x} ${ground - h}" stroke="${color}"/><ellipse cx="${x - 7}" cy="${ground - h * 0.56}" rx="7" ry="3.5" fill="${color}" transform="rotate(-28 ${x - 7} ${ground - h * 0.56})"/><ellipse cx="${x + 7}" cy="${ground - h * 0.72}" rx="7" ry="3.5" fill="${color}" transform="rotate(28 ${x + 7} ${ground - h * 0.72})"/><circle cx="${x}" cy="${ground - h}" r="${3 + Math.sqrt(value / peak) * 9}" fill="${color}"/>${weeds ? `<path class="mux-garden-weed" d="M${x + 5} ${ground} q8 -14 13 -4 q-2 -16 9 -20"/><text x="${x + 24}" y="${ground - 22}">${weeds}</text>` : ""}<text x="${x}" y="${ground + 19}" text-anchor="middle">${String(dayNumber).padStart(2, "0")}</text><title>${day.day} · ${formatLabeledValue(day.total, model.unit)}${future ? ` · scenario ${formatLabeledValue(plan.scenarioVelocity, model.unit)} / day` : ""}</title></g>`;
    })
    .join("");
  const safeY =
    ground - (14 + Math.sqrt(Math.min(peak, plan.sustainable) / peak) * 180);
...
function loomChart(model, height = 510) {
  ...
  const knots = hygiene.incidents
    ...
    .map((incident, index) => {
      ...
      const x = left + (dayIndex + 0.5) * column;
      const y = top + (originIndex + 0.5) * rowHeight;
  ...
  const readings = plan.readings
    .map((reading) => {
      const x = left + (Number(reading.date.slice(-2)) - 0.5) * column;
    ...
    .join("");
  return `<svg class="mux-loom-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Usage woven by source across days. Saturation is usage, numbered knots are possible prompt bad practices, and gold needles are manual GitHub readings."><g class="mux-loom-warp">${model.view.daily.map((day, index) => `<line x1="${left + (index + 0.5) * column}" y1="${top - 12}" x2="${left + (index + 0.5) * column}" y2="${height - 28}"/>`).join("")}</g>${threads}${origins.map((origin, index) => `<text x="${left - 12}" y="${top + (index + 0.5) * rowHeight + 4}" text-anchor="end">${escapeHtml(ORIGIN_LABELS[origin] || origin)}</text>`).join("")}${model.view.daily.map((day, index) => (index % 2 === 0 ? `<text x="${left + (index + 0.5) * column}" y="${height - 8}" text-anchor="middle">${day.day.slice(-2)}</text>` : "")).join("")}${readings}${knots}<g class="mux-loom-shuttle" transform="translate(${left + Math.min(model.view.daily.length - 1, plan.elapsed - 1) * column},${height - 23})"><path d="M0 0 h58 l13 8 -13 8 H0 l-13 -8Z"/><text x="29" y="11" text-anchor="middle">NOW</text></g></svg>`;
}
...
function boardPath(model) {
  ...
  for (const incident of hygiene.incidents)
    trapDays.set(
      incident.event.day,
      (trapDays.get(incident.event.day) || 0) + 1,
    );
  const readingDays = new Map(
    plan.readings.map((reading) => [reading.date, reading.value]),
  );
  const finishScale = Math.max(
    1,
    plan.baseBudget * 1.5,
    plan.target * 1.15,
    plan.projection * 1.05,
    plan.used * 1.05,
  );
  const finishPercent = (value) =>
    Math.max(0, Math.min(100, (value / finishScale) * 100)).toFixed(2);
  const finishStyle = `--spent:${finishPercent(plan.used)}%;--forecast:${finishPercent(plan.projection)}%;--goal-start:${finishPercent(plan.baseBudget * 0.95)}%;--goal-end:${finishPercent(plan.baseBudget * 1.05)}%;--base:${finishPercent(plan.baseBudget)}%;--approved:${finishPercent(plan.target)}%`;
  const finishImpactLabel =
    plan.unapprovedOverrun > 0
      ? plan.isFinal
        ? "UNAPPROVED DAMAGE"
        : "PROJECTED OVERRUN"
  ...
  return `<div class="mux-board" role="img" aria-label="A month board game. Each square is a day, colour is usage intensity, traps are possible prompt bad practices, and flags are manual GitHub readings.">${model.view.daily
    .map((day, index) => {
...
function classicCalendarHeatmap(model) {
  ...
  const firstWeekday =
    new Date(`${days[0]?.day || `${model.month}-01`}T12:00:00`).getDay() + 1;
  const weekdayLabels = [
    "Sunday",
    "Monday",
    "Tuesday",
...
const CUSTOM_SECTION_LABELS = {
  ...
  spikes: "Spike ranking",
  evidence: "Prompt and request evidence",
  system: "Live system state",
  sessions: "Parallel session lanes",
  resources: "Usage and resource correlation",
...
function normalizedCustomLayout(layout) {
  const source =
    layout && typeof layout === "object" ? layout : DEFAULT_CUSTOM_LAYOUT;
  const sections = (
    Array.isArray(source.sections)
      ? structuredClone(source.sections)
      : structuredClone(DEFAULT_CUSTOM_LAYOUT.sections)
  ).map((section) => ({
    ...section,
    sourceVariant: CUSTOM_SOURCE_VARIANTS.includes(section.sourceVariant)
      ? section.sourceVariant
      : "B",
  }));
  ...
  return {
    shellVariant: CUSTOM_SOURCE_VARIANTS.includes(source.shellVariant)
      ? source.shellVariant
      : "B",
    sections,
  };
...
function customTimeline(model, source) {
  const selected =
    ...
    model.view.peakSlot;
  if (source === "A0")
    return `<section class="mux-a0-timeline"><header><span>CALENDAR INTENSITY · EVERY CELL NAMES ITS VALUE</span><h2>Which days changed the burn?</h2>${legend(model)}</header>${classicCalendarHeatmap(model)}</section>`;
  if (source === "B")
    return `<section class="mux-b-scope"><div class="mux-b-scopehead"><span>Seismic ${unitLabel(model.unit)}</span><strong>${formatLabeledValue(model.view.total, model.unit)}</strong><small>Every spring is an exact 30-minute contribution.</small></div>${seismographChart(model, 330)}<div class="mux-b-selected"><span>Selected interval</span><b>Day ${selected.day.slice(-2)} · ${clock(selected.bucket * 30)}</b><strong>${formatLabeledValue(selected.total, model.unit)}</strong><small>${selected.events.length} recorded requests</small></div></section>`;
  if (source === "C")
    return `<section class="mux-c-usage"><header><span>${model.zoom === "month" ? "MONTH ORBIT" : `${model.selectedDay} ORBIT`}</span><h2>Inference orbit</h2><p>Distance and node size are usage; colour is source.</p>${legend(model)}</header>${orbitChart(model, 440)}<div class="mux-c-ruler"><span>Quiet</span><i></i><b>Selected ${model.selectedSlotId.replace(":", " / ")}</b><i></i><span>Intense</span></div></section>`;
...
function customSpikes(model, source) {
  if (source === "B")
    return `<aside class="mux-b-quakes"><header><span>Recorded shocks</span><b>${model.view.topSlots.length}</b></header>${incidentList(model, 12)}</aside>`;
  if (source === "E")
    return `<aside class="mux-e-ranking"><header><span>RANK</span><span>TIME</span><span>VALUE / CAUSE</span></header>${incidentList(model, 12)}</aside>`;
...
function customEvidence(model, source) {
  if (source === "B")
    return `<aside class="mux-b-proof">${evidence(model, 24)}</aside>`;
  if (source === "E")
    return `<section class="mux-e-selected">${evidence(model, 30)}</section>`;
...
function customSystem(model, source) {
  ...
  if (source === "E")
    return `<aside class="mux-e-machine"><header><span>HOST LEDGER</span><b data-live="verdict">${model.system.severity.toUpperCase()}</b></header>${systemStrip(model, true)}${coreRack(model)}</aside>`;
  return systemStrip(model, source !== "B");
}

function customSection(model, section) {
  const source = CUSTOM_SOURCE_VARIANTS.includes(section.sourceVariant)
    ? section.sourceVariant
    : "B";
  let content = "";
  if (section.id === "throughput")
    content = throughputPanel(model, false, source);
...
function CustomView(model, layout) {
  ...
  return `<div class="mux mux-custom" data-shell="${shell}">
    <header class="mux-custom-head"><div><span>MY MONITOR / SHELL ${shell}</span><h1>Usage, assembled.</h1><p>One live monitor composed from the prototype instruments you chose.</p></div><div class="mux-custom-period"><label>Period<select data-action="month">${model.months.map((item) => `<option value="${item}" ${item === model.month ? "selected" : ""}>${formatMonth(item)}</option>`).join("")}</select></label><div class="mux-unit">${[
      ...
      .map(
        ([key, label]) =>
          `<button data-action="unit" data-unit="${key}" class="${model.unit === key ? "is-active" : ""}" ${model.capabilities[key].available ? "" : "disabled"}>${label}</button>`,
      )
      .join(
        "",
      )}</div></div><div class="mux-custom-total"><span>${formatMonth(model.month)} total</span><strong>${model.capabilities[model.unit].available ? formatLabeledValue(model.view.total, model.unit) : `${unitLabel(model.unit)} unavailable`}</strong>${powerSwitch(model)}</div></header>
    ${controlDeck(model)}
...
async function startMonitorPrototype(config = {}) {
  ...
  setLoadingStage(
    "Contacting local monitor APIs...",
    "Reading usage timeline and live state from this Mac.",
  );
  const liveStateInitial = await waitForStoreState(store);
  setLoadingStage(
    "Decoding local records...",
    "Large local history can take longer to decode.",
  );
  const timeline = store.getLatest("usage-timeline") || { usageEvents: [] };
  let feedbackSnapshot = await (
    await fetch("/monitor/api/prototype-feedback", { cache: "no-store" })
  ).json();
  let liveState = liveStateInitial;
  const eventCount = Array.isArray(timeline?.usageEvents)
    ? timeline.usageEvents.length
    : 0;
  setLoadingStage(
    "Preparing timeline model...",
    `${eventCount.toLocaleString()} usage records found locally.`,
  );
  ...
  const mergeLiveEvents = () => {
    ...
    events = [
      ...events,
      ...[...liveEvents.values()].filter((event) => !durableIds.has(event.id)),
    ].sort((left, right) => Date.parse(left.at) - Date.parse(right.at));
  };
  ...
  let preferredView = feedbackSnapshot.preferredView || {
    mode: "classic",
    variant: "B",
    layout: { shellVariant: "B", sections: [] },
  };
  const openPreferred = params.get("view") === "preferred";
  let customMode =
    params.get("layout") === "custom" ||
    (openPreferred && preferredView.mode === "custom");
  const requestedVariant = params.get("variant")?.toUpperCase();
  let variant = VARIANTS[requestedVariant]
    ? requestedVariant
    : openPreferred &&
        preferredView.mode === "variant" &&
        VARIANTS[preferredView.variant]
      ? preferredView.variant
      : customMode
        ? normalizedCustomLayout(preferredView.layout).shellVariant
        : config.variant && VARIANTS[config.variant]
          ? config.variant
          : "B";
  if (requestedVariant === "G") customMode = true;
  let unit = ["tokens", "credits", "dollars"].includes(params.get("unit"))
    ? params.get("unit")
    : "credits";
  let month = months.includes(params.get("month"))
    ? params.get("month")
    : months.reduce(
        (latest, item) =>
          events.filter((event) => event.month === item).length >= 10
            ? item
            : latest,
        months.at(-1),
      );
  let referenceMonth =
    months.includes(params.get("reference")) &&
    params.get("reference") !== month
  ...
  let zoom = ["month", "day", "slot", "session"].includes(params.get("zoom"))
    ? params.get("zoom")
    : "month";
  let anomalyMode = ["value", "jump", "unusual"].includes(params.get("anomaly"))
    ? params.get("anomaly")
    : "value";
  let threshold = Math.max(0, Number(params.get("threshold")) || 0);
  let sortDirection = globalThis.SaxjaxDateTimeSort.normalize(
    params.get("sort"),
  );
  ...
  const forecastTokenTotal =
    Number(liveState.copilot?.forecast?.inputTokens || 0) +
    Number(liveState.copilot?.forecast?.outputTokens || 0);
  const initialTokenTarget = Math.max(
    1,
    Math.ceil(
      Math.max(
        forecastTokenTotal,
        Number(liveState.copilot?.observedInputTokens || 0) +
          Number(liveState.copilot?.observedOutputTokens || 0),
      ) / 10_000,
    ) * 10_000,
  );
  let baseBudgets = {
  ...
  function rememberSystemSample() {
    liveSamples.push({
      ...
      system: systemView(liveState),
    });
    if (liveSamples.length > 3600)
      liveSamples.splice(0, liveSamples.length - 3600);
  }
  ...
  function capabilityModel(filteredEvents) {
    ...
    const nativeMeasured = selected.filter((event) =>
      Number.isFinite(event.credits),
    );
    const nativeProviders = [
      ...new Set(nativeMeasured.map((event) => event.provider)),
    ];
    const tokenMeasured = selected.filter((event) =>
      Number.isFinite(event.tokens),
    );
    const moneyMeasured = selected.filter((event) =>
      Number.isFinite(event.dollars),
    );
    const nativeCompatible = nativeProviders.length <= 1;
    const providers = [...new Set(selected.map((event) => event.provider))];
    const evidenceLabel = (measured) =>
      measured.length === selected.length && measured.length
        ? "client-reported"
        : measured.length
          ? "partial client record"
          : "not supplied";
    return {
      tokens: {
        available: tokenMeasured.length > 0,
        confidence: evidenceLabel(tokenMeasured),
        note: `${tokenMeasured.length.toLocaleString()} of ${selected.length.toLocaleString()} requests include token evidence.`,
      },
      credits: {
        available: nativeMeasured.length > 0 && nativeCompatible,
        confidence: nativeCompatible
          ? evidenceLabel(nativeMeasured)
          : "not aggregatable",
        note: nativeCompatible
          ? `${nativeMeasured.length.toLocaleString()} of ${selected.length.toLocaleString()} requests include the provider's native unit.`
          : "Multiple providers use different native units; choose one provider before aggregating.",
      },
      dollars: {
        available:
          moneyMeasured.length > 0 &&
          moneyMeasured.length === nativeMeasured.length,
        confidence: moneyMeasured.length
          ? "local prototype estimate"
          : "not supplied",
        note: moneyMeasured.length
          ? `${moneyMeasured.length.toLocaleString()} requests use the prototype Copilot rule of 1 credit = $0.01; this is not authoritative billing.`
          : "No versioned money conversion is available for this selection.",
      },
      providers,
    };
  }
  ...
  function filteredTimelineEvents() {
    ...
    return events.filter((event) => {
      ...
      if (filters.origin !== "all" && event.origin !== filters.origin)
        return false;
      if (filters.model !== "all" && event.model !== filters.model)
        return false;
      if (
        search &&
        !`${event.prompt} ${event.source} ${event.sessionId} ${event.model}`
          .toLocaleLowerCase()
          .includes(search)
      )
        return false;
  ...
  function model() {
    ...
    const referenceView = referenceMonth
      ? buildView(filteredEvents, referenceMonth, unit, anomalyMode, threshold)
      : null;
    if (!view.slots.some((slot) => slot.id === selectedSlotId))
      selectedSlotId = view.peakSlot.id;
    if (!view.days.includes(selectedDay))
      selectedDay =
        view.slots.find((slot) => slot.id === selectedSlotId)?.day ||
        view.peakDay.day;
    if (
      selectedSessionId &&
      !view.monthEvents.some((event) => event.sessionId === selectedSessionId)
    )
      selectedSessionId = null;
    if (
      selectedEvidenceEventId &&
      !view.monthEvents.some((event) => event.id === selectedEvidenceEventId)
    )
    ...
    const providerLabels = Object.fromEntries(
      providers.map((provider) => [
        provider,
        PROVIDER_RULES[provider]?.label || provider,
      ]),
    );
    const billingReads =
      capabilities.providers.length > 0 &&
      capabilities.providers.every(
        (provider) => PROVIDER_RULES[provider]?.billingReads === true,
      );
    return {
      variant,
      customMode,
      ...
      budgetAdditionDate,
      paceScenario,
      traffic: [...(liveState.history || []), ...(liveState.active || [])],
  ...
  function render() {
    ...
    const evidenceSnapshot =
      openPaper && openSheet?.dataset.loaded === "true"
        ? {
            eventId: openPaper.dataset.eventId,
            input:
              openSheet.querySelector("[data-paper-input]")?.textContent || "",
            output:
              openSheet.querySelector("[data-paper-output]")?.textContent || "",
            reasoning:
              openSheet.querySelector("[data-paper-reasoning]")?.textContent ||
              "",
            status:
              openSheet.querySelector("[data-paper-status]")?.textContent ||
              "Actual local journal loaded",
          }
        : null;
    const boxSnapshot =
      openPaper && openSheet
        ? {
            ...
            open: Object.fromEntries(
              [...openSheet.querySelectorAll(".mux-paper-box[data-box]")].map(
                (box) => [box.dataset.box, box.open],
              ),
            ),
          }
        : null;
    ...
    const focusSnapshot = activeField
      ? {
          formAction:
            activeField.closest("form[data-action]")?.dataset.action || null,
    ...
    const view = customMode
      ? CustomView(current, preferredView.layout)
      : views[variant](current);
    ...
    if (evidenceSnapshot) {
      replacementPaper =
        [...root.querySelectorAll(".mux-evidence-paper")].find(
          (paper) => paper.dataset.eventId === evidenceSnapshot.eventId,
        ) || null;
      const replacementSheet = replacementPaper?.querySelector(
        ".mux-evidence-sheet",
      );
      if (replacementSheet) {
        ...
        const reasoning = replacementSheet.querySelector(
          "[data-paper-reasoning]",
        );
    ...
    if (boxSnapshot) {
      const paper = [...root.querySelectorAll(".mux-evidence-paper")].find(
        (candidate) => candidate.dataset.eventId === boxSnapshot.eventId,
      );
    ...
    document.title = customMode
      ? "My mixed monitor — Saxjax"
      : `${variant} — ${VARIANTS[variant]} — Saxjax ${labEnabled ? "prototype" : "Monitor"}`;
    bind();
    ...
    if (focusSnapshot) {
      const selector =
        focusSnapshot.formAction && focusSnapshot.name
          ? `form[data-action="${focusSnapshot.formAction}"] [name="${focusSnapshot.name}"]`
          : focusSnapshot.dataAction
            ? `[data-action="${focusSnapshot.dataAction}"]`
            : null;
  ...
  function syncUrl() {
    ...
    params.delete("view");
    if (customMode) params.set("layout", "custom");
    else params.delete("layout");
    params.set("unit", unit);
    params.set("month", month);
    params.set("day", selectedDay);
    params.set("slot", selectedSlotId);
    if (referenceMonth) params.set("reference", referenceMonth);
    else params.delete("reference");
    if (selectedSessionId) params.set("session", selectedSessionId);
    else params.delete("session");
    if (selectedEvidenceEventId) params.set("request", selectedEvidenceEventId);
    else params.delete("request");
    if (openPromptEventId) params.set("prompt", openPromptEventId);
  ...
  function moveSlot(key) {
    const current = model();
    const index = current.view.slots.findIndex(
    ...
    const next =
      current.view.slots[
        Math.max(0, Math.min(current.view.slots.length - 1, index + delta))
      ];
    if (!next) return;
    ...
    requestAnimationFrame(() =>
      root.querySelector("[data-timeline-keyboard]")?.focus(),
    );
  }
  ...
  function bind() {
    ...
    const revealEvidencePaper = (details) =>
      requestAnimationFrame(() => {
        if (!details?.open) return;
        ...
        list.scrollTop = Math.max(
          0,
          list.scrollTop + paperBox.top - listBox.top - 12,
        );
      });
    const hydrateEvidencePaper = async (details) => {
      ...
      if (
        !details.dataset.captureClient ||
        !details.dataset.captureSession ||
        !details.dataset.captureRequest
      ) {
        if (status)
          status.textContent =
            "Full journal identity is unavailable; showing the durable excerpt.";
        sheet.dataset.loaded = "true";
        return;
      }
      sheet.dataset.loading = "true";
      if (status)
        status.textContent = "Opening the original local request journal…";
      try {
        const query = new URLSearchParams({
          client: details.dataset.captureClient,
          session: details.dataset.captureSession,
          request: details.dataset.captureRequest,
        });
        const response = await fetch(`/monitor/api/request-capture?${query}`, {
          cache: "no-store",
        });
        const capture = await response.json();
        if (!response.ok)
          throw new Error(capture.error || `HTTP ${response.status}`);
        ...
        if (input)
          input.textContent =
            capture.input ||
            capture.submitted ||
            "No locally retained input content.";
        if (output)
          output.textContent =
            capture.output || "No locally retained output content.";
        if (reasoning && capture.reasoning) {
          reasoning.textContent = capture.reasoning;
          reasoning.closest("details").hidden = false;
        }
        if (status)
          status.textContent = `Original local journal loaded · ${capture.inputStatus || "captured"}`;
        sheet.dataset.loaded = "true";
      } catch (error) {
        if (status)
          status.textContent = `${error.message}. The durable excerpt remains visible.`;
      } finally {
    ...
    const releaseEvidencePaper = (details) => {
      ...
      if (input)
        input.textContent =
          details.dataset.paperPreview ||
          "Durable request excerpt unavailable.";
      if (output)
        output.textContent = details.dataset.captureRequest
          ? "Open the paper to load the complete output…"
          : "The complete output is not retained in this timeline record.";
      if (reasoning) {
        reasoning.textContent = "";
        reasoning.closest("details").hidden = true;
      }
      if (status)
        status.textContent = details.dataset.captureRequest
          ? "Open FORENSICS to read the original local journal"
          : "Only the durable timeline excerpt is available";
      sheet.dataset.loaded = "false";
    };
    root.querySelectorAll(".mux-prompt-case").forEach((details) =>
    ...
    root.querySelectorAll('[data-action="controls-toggle"]').forEach((button) =>
      button.addEventListener("click", () => {
        controlsOpen = !controlsOpen;
        render();
      }),
    );
    root.querySelectorAll('[data-action="detail-level"]').forEach((button) =>
      button.addEventListener("click", () => {
        prototypeDetailLevel =
          prototypeDetailLevel === "full" ? "essential" : "full";
        try {
          localStorage.setItem(PROTOTYPE_DETAIL_KEY, prototypeDetailLevel);
        } catch {}
        render();
      }),
    );
    root.querySelectorAll('[data-action="unit"]').forEach((button) =>
      button.addEventListener("click", () => {
    ...
    root.querySelectorAll('[data-action="month"]').forEach((select) =>
      select.addEventListener("change", () => {
        month = select.value;
        if (referenceMonth === month) referenceMonth = null;
        selectedDay = null;
        selectedSlotId = null;
        selectedSessionId = null;
        selectedEvidenceEventId = null;
    ...
    root.querySelectorAll('[data-action="view-month"]').forEach((button) =>
      button.addEventListener("click", () => {
    ...
    root.querySelectorAll('[data-action="day"]').forEach((element) =>
      element.addEventListener("click", () => {
        selectedDay = element.dataset.day;
        const daySlots = model().view.slots.filter(
          (slot) => slot.day === selectedDay,
        );
        selectedSlotId = daySlots.reduce(
          (peak, slot) => (slot.total > peak.total ? slot : peak),
          daySlots[0],
        ).id;
        selectedSessionId = null;
    ...
    root.querySelectorAll('[data-action="slot"]').forEach((element) =>
      element.addEventListener("click", (event) => {
        event.stopPropagation();
        selectedSlotId = element.dataset.slot;
        selectedDay = selectedSlotId.slice(0, 10);
        selectedSessionId = null;
        selectedEvidenceEventId = null;
        zoom = "slot";
        syncUrl();
    ...
    root.querySelectorAll('[data-action="session"]').forEach((element) =>
      element.addEventListener("click", () => {
        ...
        const first = model().view.monthEvents.find(
          (event) => event.sessionId === selectedSessionId,
        );
        if (first) {
          selectedDay = first.day;
          selectedSlotId = `${first.day}:${first.bucket}`;
        }
        zoom = "session";
        syncUrl();
        render();
    ...
    root.querySelectorAll('[data-action="filter"]').forEach((select) =>
      select.addEventListener("change", () => {
    ...
    root.querySelectorAll('[data-action="search"]').forEach((input) =>
    ...
    root.querySelectorAll('[data-action="reference"]').forEach((button) =>
      button.addEventListener("click", () => {
        referenceMonth =
          referenceMonth === button.dataset.month ? null : button.dataset.month;
        syncUrl();
        render();
      }),
    );
    root
      .querySelectorAll('[data-action="reference-select"]')
      .forEach((select) =>
        select.addEventListener("change", () => {
          referenceMonth = select.value || null;
          syncUrl();
          render();
        }),
      );
    root.querySelectorAll('[data-action="anomaly"]').forEach((button) =>
      button.addEventListener("click", () => {
        anomalyMode = button.dataset.mode;
        selectedSlotId = null;
        selectedSessionId = null;
        selectedEvidenceEventId = null;
        syncUrl();
    ...
    root.querySelectorAll('[data-action="threshold"]').forEach((input) =>
      input.addEventListener("change", () => {
        threshold = Math.max(0, Number(input.value) || 0);
        selectedSlotId = null;
        selectedSessionId = null;
        selectedEvidenceEventId = null;
        syncUrl();
    ...
    root.querySelectorAll('[data-action="zoom"]').forEach((button) =>
      button.addEventListener("click", () => {
    ...
    root.querySelectorAll('[data-action="sort"]').forEach((button) =>
      button.addEventListener("click", () => {
        sortDirection = globalThis.SaxjaxDateTimeSort.normalize(
          button.dataset.sort,
        );
        syncUrl();
        render();
        window.dispatchEvent(
          new CustomEvent("saxjax-date-sort-change", {
            detail: { direction: sortDirection },
          }),
        );
      }),
    );
    root
      .querySelectorAll('form[data-action="manual-reading"]')
      .forEach((form) =>
        form.addEventListener("submit", (event) => {
          ...
          manualReadings = [
            ...manualReadings.filter(
              (reading) => reading.date !== date || reading.unit !== unit,
            ),
            { id, date, unit, value },
          ];
          feedbackLab?.track(
            "planning.github-reading",
            "manual-github-readings",
          );
          render();
        }),
      );
    root.querySelectorAll('[data-action="remove-reading"]').forEach((button) =>
      button.addEventListener("click", () => {
        manualReadings = manualReadings.filter(
          (reading) => reading.id !== button.dataset.readingId,
        );
        feedbackLab?.track(
          "planning.github-reading-remove",
          "manual-github-readings",
        );
        render();
      }),
    );
    root.querySelectorAll('[data-action="base-budget"]').forEach((input) =>
      input.addEventListener("change", () => {
        baseBudgets = {
          ...baseBudgets,
          [unit]: Math.max(
    ...
    root
      .querySelectorAll('form[data-action="budget-addition"]')
      .forEach((form) =>
        form.addEventListener("submit", (event) => {
          event.preventDefault();
          const data = new FormData(form);
          ...
          budgetAdditions = [
            ...budgetAdditions,
            { id: `${unit}:${date}:${Date.now()}`, month, date, unit, value },
          ];
    ...
    root
      .querySelectorAll('[data-action="remove-budget-addition"]')
      .forEach((button) =>
        button.addEventListener("click", () => {
          budgetAdditions = budgetAdditions.filter(
            (addition) => addition.id !== button.dataset.budgetAdditionId,
          );
          feedbackLab?.track(
            "planning.budget-addition-remove",
            "course-control",
          );
          render();
        }),
      );
    root.querySelectorAll('[data-action="pace-scenario"]').forEach((input) =>
      input.addEventListener("input", () => {
        paceScenario = Math.max(20, Math.min(180, Number(input.value) || 100));
        render();
      }),
    );
    root.querySelectorAll('[data-action="layout-studio"]').forEach((button) =>
      button.addEventListener("click", () => {
        if (feedbackLab) feedbackLab.openStudio();
        else
          location.assign(
            "/monitor/?prototype=monitor&view=preferred&studio=layout",
          );
      }),
    );
    ...
    root.querySelectorAll('[data-action="copy-source"]').forEach((button) =>
      button.addEventListener("click", async () => {
        const source = events.find(
          (event) => event.id === button.dataset.event,
        )?.source;
        if (!source) return;
    ...
    root.querySelectorAll('[data-action="close-forensics"]').forEach((button) =>
      button.addEventListener("click", () => {
    ...
    root.querySelectorAll("[data-timeline-keyboard]").forEach((element) =>
  ...
  function updateLive() {
    ...
    const values = {
      ...
      memory: `${Math.round(system.memory)}%`,
      load: `${Math.round(system.loadRatio * 100)}%`,
      ollamaCpu: `${Math.round(system.ollamaCpu)}%`,
      model: system.model,
      modelMeta: `${system.modelSize} · ${system.processor}`,
      verdict:
        system.severity === "clear"
          ? "HEADROOM"
          : system.severity === "busy"
            ? "BUSY"
            : "PRESSURE",
      server: `Ollama ${system.online ? "online" : "offline"}`,
    };
    for (const [key, value] of Object.entries(values))
      root.querySelectorAll(`[data-live="${key}"]`).forEach((element) => {
        element.textContent = value;
      });
    const bars = {
      cpu: system.cpu,
      memory: system.memory,
      load: system.loadRatio * 100,
      ollamaCpu: system.ollamaCpu,
    };
    for (const [key, value] of Object.entries(bars))
      root
        .querySelectorAll(`[data-live-bar="${key}"]`)
        .forEach((element) =>
          element.style.setProperty("--fill", `${Math.min(100, value)}%`),
        );
    root.querySelectorAll("[data-severity]").forEach((element) => {
      element.dataset.severity = system.severity;
    });
    root.querySelectorAll('[data-action="ollama-power"]').forEach((button) => {
      button.classList.toggle("is-online", system.online);
      button.setAttribute("aria-checked", String(system.online));
      const label = button.querySelector("b");
      if (label && !powerPending)
        label.textContent = system.online ? "Ollama online" : "Ollama offline";
    });
  }
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (
      ...
      document.querySelector(
        ".pfl-comparison, .pfl-composer, .pfl-default-menu, .pfl-studio",
      )
    )
      return;
    if (
      target.matches(
        "input, textarea, select, [contenteditable], [data-timeline-keyboard]",
      )
    )
      return;
  ...
  store.on("selection", (id) => {
    ...
    if (surfaceActive()) render();
    else dirty = true;
  });
  window.addEventListener("saxjax-surface-change", () => {
    if (surfaceActive() && dirty) {
      dirty = false;
      render();
    }
  });
  ...
  if (labEnabled) {
    const { createPrototypeFeedbackLab } =
      await import("/monitor/prototype-feedback-lab.js");
    feedbackLab = await createPrototypeFeedbackLab({
      prototypeRoot: root,
      variants: VARIANTS,
      initialState: feedbackSnapshot,
      getContext: () => ({
        variant: customMode ? "CUSTOM" : variant,
        custom: customMode,
        month,
        unit,
        zoom,
...
function ensureStarted(config = {}) {
  if (!startedPromise) {
    startedPromise = startMonitorPrototype(config).catch((error) => {
      ...
      const root =
        document.querySelector("#monitor-ux-prototype-root") ||
        document.body.appendChild(document.createElement("div"));
      root.id = "monitor-ux-prototype-root";
      root.style.cssText =
        "display:block;padding:40px;color:#f5d8cf;background:#1b1110;font:16px/1.5 ui-monospace,monospace;min-height:100vh";
